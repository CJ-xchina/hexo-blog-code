...
*1)* ***`reshape`***
不改变原数组元素，返回一个新的shape维度的数组(维度变换)。`reshape` 通常的用法是：`arr.reshape(n1,n2,n3,....,nw)` ，其功能是将$arr$向量转换为$w$ 维度，且维度从高到低拥有的元素数量是：$n1,n2,n3,...,nw$
...
| `ndarray.cumsum( axis=0) `<br/> `ndarray.cumprod( axis=0)` | 累加 累乘                        |
| `ndarray.std()`<br />`ndarray.var()`                       | 方差标准差                       |
| `ndarray.max()`<br />`ndarray.min()`                       | 最大值最小值                     |
| `ndarray.argmax()`<br />`ndarray.argmin()`                 | 最大值索引最小值索引             |
