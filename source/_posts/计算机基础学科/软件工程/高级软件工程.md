...
可以使用begin和rescue关键字来捕获异常。begin块中的代码被监视，如果发生异常，会跳转到rescue块，并执行相关的处理代码。 
...
begin
 ...
 例如，下面的代码捕获RuntimeError和ArgumentError两种异常：
begin
  # 可能会引发异常的代码
rescue RuntimeError
...
这些是Ruby中异常处理的基本概念和用法。通过使用raise抛出异常，begin、rescue和ensure来捕获和处理异常，可以有效地处理错误和异常情况，提高程序的健壮性和可靠性.
3.3 Ruby 面向对象
定义对象
...
2. 在类中定义实例变量作为对象的属性。实例变量以@符号开头，可以在类的任何方法中使用。例如：
class Person
...
3. 在类中定义方法作为对象的行为。方法是类中的函数，可以执行特定的操作。例如：
class Person
  def initialize(name, age)
    @name = name
    @age = age
...
上述例子中，initialize方法可以看作构造函数，构造函数使用`@` 关键字修饰的变量可以看作类的属性，同时也可以显示定义类的属性，使用关键字@@修饰，例如：
class MyClass
  @@class_var = 20
end

puts MyClass.class_variables
运行上述代码会输出@@class_var的值，它是`MyClass`类的一个类变量。
4. 使用类的new方法创建对象。new方法是类的构造函数，用于实例化对象并调用initialize方法进行初始化。例如：
person = Person.new("John", 25)
5. 可以通过对象调用类中定义的方法。例如：
person.introduce
这将输出：My name is John and I am 25 years old.

封装对象
在Ruby中，类的封装是通过访问控制符（Access Control）来实现的。Ruby提供了三种访问控制符，分别是public、private和protected，用于控制类中方法和属性的可见性。
...
2. private：私有方法只能从类的内部调用，无法从外部直接访问。私有方法不能被类的实例直接调用，只能通过类的内部方法来间接调用。
class MyClass
  def public_method
    puts "This is a public method"
...
3. protected：受保护方法可以从类的内部以及该类的实例的上下文中访问。受保护方法可以被类的实例直接调用，也可以在同一个类的其他实例方法中调用。
class MyClass
  def public_method
    puts "This is a public method"
