...
- **对于隐藏层的激活函数：**通常使用`ReLU`激活函数，下面介绍原因
  1. `ReLU`相较于`sigmoid`激活函数，其运算速度更快
