...
<img src="https://img-blog.csdnimg.cn/5c249ea864054830853d769a676880b6.png" alt="在这里插入图片描述" style="zoom:50%;" />
在每个节点上，当选择用于拆分的特征时，如果n个特征可用，则选择一个**随机子集** $K$ 个特征，并允许算法仅从该特征子集中进行选择，往往用于具有较多特征的较大问题。
当n很大时，通常令$K=\sqrt{n}$​。

### XGBoost

**XGBoost(Extreme Gradient Boosting)**是一种增强决策树算法，运行速度快，开源易于使用，且非常成功的赢得了很多机器学习比赛，并成功应用于很多商业应用中。
对生成树集合的算法进行了改进，除了第一次取样以外，与其以相等的概率从所有示例中选择实例，不如**优先选择以前训练过的树错误分类的示例**，也被称为**<font color='red'>刻意练习</font>**，思想是将注意力集中在容易出错的小部分。
